...
import finn.artifact_rejection.outlier_removal
...
def run(data, ch_names, fs, ref_areas = [[105, 120], [135, 145], [155, 195]], broadness = 3, visual_inspection = True):
    """
    Identifices which channel have substantially more or less power in frequency ranges defined by ref_areas. Channels whose power is more different than
    broadness (default: 3) standard deviations will be primed as faulty channels. In case visual inspection is active (default: True), the automatic results
    can be further refined via manual selection.
    ...
    @param ref_areas: spectral reference areas used for power estimation. Only power within these ranges is defined. It is recommended to choose ranges, 
    which are not part of any evaluation to decrease the chances of pre-processing induced biases.
    ...
    if (visual_inspection):
        (valid_list, invalid_list) = manual_check(len(data), ch_names, diff, min_ref, max_ref, valid_list, invalid_list)
    idenfity_faulty_visual_inspection_lock.release()
        
    return (valid_list.tolist(), invalid_list.tolist(), np.asarray(score_list))

def manual_check(ch_cnt, ch_names, score, min_ref, max_ref, valid_list, invalid_list):
    ...
    vis_sub_process = multiprocessing.Process(target = __manual_check, args = [ch_cnt, ch_names, score, min_ref, max_ref, valid_list, invalid_list, shared_valid_list])
...
def __manual_check(ch_cnt, ch_names, score, min_ref, max_ref, valid_list, invalid_list, shared_valid_list):    
    ...
    win = Qt_win(app, ch_cnt, ch_names, min_ref, max_ref, score, valid_list, invalid_list)
...
class Qt_win(PyQt5.QtWidgets.QWidget):
