...
import numpy as np
import finn.artifact_rejection.neighboring_channels as nc
...
def run(data, ch_names, bad_ch_idx_list):
    """
    Restores channel by averaging signals from their respective neighbors. In case neighboring channels are flagged as bad, channels get
    iteratively restored, from the channel with the most valid neighbors to the channel with the least. Restored channels are considered
    valid candidates for channel reconstruction.
    
    @param data: input data in the format channels x samples
    @para ch_names: Names of the channles from the input data. Order needs to be aligned with the channel order of ch_names.
