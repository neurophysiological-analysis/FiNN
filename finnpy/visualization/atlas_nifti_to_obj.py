...
import finnpy.visualization._conv_nifti_obj  # @UnresolvedImport
...
def conv_ewert(mat_path, nii_path, obj_path, slicer_path,
    ...
    for region_idx in range(file["atlases"]["names"].shape[0]):  # pylint: disable=no-member
...
def conv_hybra(mat_path, nii_path, obj_path, slicer_path,
               seg_min_thresh = None, seg_max_thresh = None):
    """
    Extracts subcortical components from the HybraPD 2021 atlas.
    ...
    for region_idx in range(file["atlases"]["names"].shape[0]):  # pylint: disable=no-member
        ...
        ex_c.run("01", [os.path.dirname(__file__) + "/slicer.sh", slicer_path,
                        finnpy.visualization._conv_nifti_obj.__file__,  # pylint: disable=protected-access
                        (nii_path + f_name), obj_path, f_name.split(".")[0], str(seg_min_thresh), str(seg_max_thresh)])
