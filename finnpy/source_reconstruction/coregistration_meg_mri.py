...
def load_hd_surf(subj_name, subj_path):
    """
...
def load_mri_ref_pts(subj_path, subj_name):
    """
...
def rm_bad_head_shape_pts(meg_pts, mri_pts, trans_mat, distance_thresh = 5/1000):
    """
...
def run(subj_name, subj_path, rec_meta_info, registration_scale_type = "restricted",
        max_number_of_iterations = 500):
    """
    ...
    meg_pts = load_meg_ref_pts(rec_meta_info)
    mri_pts = load_mri_ref_pts(subj_path, subj_name)
    hd_surf_vert = load_hd_surf(subj_name, subj_path)
    ...
    (coreg_rotors, coreg_mat) = registrate_3d_points_restricted(mri_pts_initial, meg_pts_initial, scale = (registration_scale_type == "free"))
