...
def butter(data, f_low, f_high, fs, order, zero_phase = True):
    ...
    Parameters
    ----------
    data: list or np.ndarray, len(sample_cnt)
          Single vector of input data    
    f_low : float
            Start of filters's pass band.
    f_high : float
             End of the filters's pass band.
    fs : float
         Sampling frequency
    order : int
            Order of the filters.
    zero_phase : boolean
                 If 'true' the filters is applied forwards and backwards resulting in zero phase distortion.
    ...
    filter_func = _apply_zero_phase if (zero_phase is True) else _apply_nonzero_phase

    return filter_func(filter_param[0], filter_param[1], data)
        
def _apply_zero_phase(b, a, data): 
    """
    Apply the filters twice resulting in zero phase distortion.
    
    Parameters
    ----------
    b : list or np.ndarray, len(numerator_coefficient_cnt,)
        Numerator of the filters's coefficient vector
    a: list or np.ndarray, len(denominator_coefficient_cnt,)
       Denominator of the filters's coefficient vector
    data : list or np.ndarray, shape(sample_cnt, )
           The data array
...
def _apply_nonzero_phase(b, a, data):
    """
    Apply scipy's lfilter function.
...
def fir(data, f_low, f_high, trans_width, fs, ripple_pass_band = 10e-5, stop_band_suppression = 10e-7, fft_win_sz = 2, pad_type = "zero", 
        mode = "fast"):
    """
    Construct an FIR filter and applies it to data.
    ...
    ----------
    data : list or np.ndarray, len(sample_cnt,)
           Data to be filtered. Single vector of data.
    f_low : float
            Start of the pass band.
    f_high : float
    ...
    fs : float
         Sampling frequency.
    ripple_pass_band : float
                       Suppression of the ripple in the pass band.
    stop_band_suppression : float
                            Suppression of the stop band.
    fft_win_sz: int
                Size of the fft window.
    pad_type : str
               Type of padding. Supported types are: 'zero' and 'mirror'.
    mode : str
           Defines whether the FIR filter is executed in 'fast' or 'precise' mode (32 vs 64 bit floats)
    ...
    if (f_low is not None and f_high is not None):  # Is band pass or band stop
        if (f_low > f_high):  # Is band stop
            (freq, gain) = _calc_FIR_freq_and_gain_two_sided(nyq, f_high, f_low, trans_width)  # pylint: disable=arguments-out-of-order
    ...
    coeffs = scipy.signal.firwin2(numtaps = N, freq = freq, gain = gain, window = 'hamming')  # Guarantees a linear phase pylint: disable=possibly-used-before-assignment
    if (mode == "fast"):
...
def _estimate_FIR_coeff_num(fs, trans_width, ripple_pass_band = 10e-5, stop_band_suppression = 10e-7):
    """
    Estimate the FIR filters coefficients base on the given parameters.
    
    Note, the sharper the 'edges' of the filters are, the wider becomes the filters
    ...
    trans_width : float
                  Width of the transition band
    ripple_pass_band : float
                       Factor for suppression of the ripple in the pass band
    stop_band_suppression : float
                            Factor for suppression of the ripple in the stop band
...
def _calc_FIR_freq_and_gain_two_sided(nyq, f_low, f_high, trans_width):
    """
    Calculate the two-sided frequency window and the gain for a FIR filters.
    ...
    f_low : float
            Low pass frequency of the filters
    f_high: float
            High pass frequency of the filters
    trans_width : float
                  Width of the transition band
    ...
    -------
    tuple of (list, list)
        - Frequency window : list
                             Frequency bins for the filter coefficients.
        - Gain window : list
                        Gain windows for the filter coefficients.
    """
...
def _calc_FIR_freq_and_gain_left_sided(nyq, f_low, trans_width):
    """
    Calculate the left-sided frequency window and the gain for a FIR filters.
...
def _calc_FIR_freq_and_gain_right_sided(nyq, f_high, trans_width):
    """
    Calculate the right-sided frequency window and the gain for a FIR filters.
...
def _overlap_add(data, coeffs, fs, trans_width, fft_win_sz, pad_type):
    """
    Apply the overlap add approach to speed up filter application.
    ...
    fft_win_sz = _overlap_add_sanity_check(fs = fs, trans_width = trans_width, filter_length = coeff_length, 
    ...
    for segIdx in range(0, seg_cnt):
        ...
        # abrupt stop.
        result_data[filtStart:filtEnd] += seg_result_data[tmpStart:tmpEnd] 
    
    # Remember to remove the padded data from the new data.
    # The left pad should already be removed, but the 
    # "padded_data" reference still 'counts' for two pads.
    res_data = result_data[:len(padded_data) - 2 * pad_width].astype(data.dtype)

    return res_data

def _overlap_add_sanity_check(fs, trans_width, filter_length, shift, fft_win_sz = 2):
    """
    Check whether designed filters can be applied to the data.
    ...
    shift : int
            Size of the filters induced shift.
    fft_win_sz: int
                Size of the fft window.
...
def _estimate_overlap_add_segments(data_length, pad_edge_width, filter_length, fft_win_sz):
    """
    Estimate the size and the number of segments used in the overlap add approach.
    
    Parameters
    ----------
    data_length : int
                  Lenght of the data in samples.
    pad_edge_width : int
                     Length of the padding on the left and right side of the data.
    filter_length : int
...
def _pad_overlap_add_data(data, pad_width, pad_type = "zero"):
    """
    Pad the individual segments of the overlap add function.
