...
def run(src_data, subj_to_fsavg_mdl, fs_path):
    """
    Calculate source region averaging.
    
    Space regions are averaged as identified in the Desikan-Killiany atlas.
    ...
    subj_to_fsavg_mdl : finnpy.src_rec.subj_to_fsavg.Subj_to_fsavg_mdl
                        Container with objects for the projection from a specific subject to fs average:
    ...
    Returns
    -------
    result: tuple of (numpy.ndarray, list of lists, list)
            ...
            - morphed_region_names : list, len(source_ch_cnt,)
                                     Region names.
    """
    fs_avg_path  = (fs_path + "/") if (fs_path[-1] != "/") else fs_path  # noqa: E221
    ...
    hem_offset = [0]
    for (hem_idx, hemisphere) in enumerate(["lh", "rh"]):
        ...
        for region_name in regions.keys():
            ...
            morphed_epoch_channels.append(model_region_vertices_ids + hem_offset[hem_idx])
            morphed_region_names.append(hemisphere + "_" + region_name)
        hem_offset.append(np.sum(loc_valid_vert))
