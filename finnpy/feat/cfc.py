...
def dmi(low_freq_data, high_freq_data,
            phase_window_half_size = 10, phase_step_width = 1,
            max_model_fit_iterations = 200):
    """
    Calculate the direct modulation index.
    
    Calculates the direct modulation index between a low frequency signal and a high frequency signal.
    Instead of the original modulation index based on entropy, this modulation index estimate is based on a sinusoidal fit. 
    
    Parameters
    ----------
    low_freq_data : np.ndarray, shape(n_samples,)
                    Single array of low frequency data.
    high_freq_data : np.ndarray, shape(n_samples) 
                     Single array of high frequency data. Must have the same length as low_freq_data.
    phase_window_half_size : int
                             Width of the phase window used for calculation of frequency/phase histogram.
                             Amplitude gets added to every phase bin within the window size.
                             Larger windows result in more smooth, but also potentially increased PAC estimates.
    phase_step_width : int
                       Step width of the phase window used for calculation of frequency/phase histogram.
    max_model_fit_iterations : int
                               Maximum number of iterations applied during sine fitting.
    ...
    tuple of (float, np.ndarray, np.ndarray)
        ...
        - idealized_pac_shape : np.ndarray, shape(n_bins,)
                                Idealized shape of the PAC curve.
        - real_pac_shape : np.ndarray, shape(n_bins,)
                           Realisitc shape of the PAC curve.
    
    Raises
    ------
    AssertionError
        If the low and high frequency signals have different length.
    """
    ...
    amplitude_signal  = amplitude_signal * 2 - 1  # noqa: E221
    ...
    if (np.isnan(amplitude_signal).any()):
        amplitude_signal = np.where(np.isnan(amplitude_signal) is False)[0]
...
def _sine(x, phase, amp):
    ...
    Internal method. Used in dmi to estimate the direct modulation index.
    The amount of PAC is quantified via a sine fit. This sine is defined by the following paramters:
    
    Parameters
    ----------
    x : np.ndarray, shape(n_bins)
        Samples to fit a sine against.
    phase : float
            Phase shift of the sine.
    amp : float
    ...
    np.ndarray, shape(n_bins)
        Returns the fitted sine at the locations indicated by x.
    """
    freq = 1
    fs = 1
    return amp * (np.sin(2 * np.pi * freq * (x - ((phase + 180) / 360)) / fs))
    
def plv(low_freq_data, high_freq_data):
    """
    Calculate the phase lock value between a low frequency signal and a high frequency signal.
    ...
    low_freq_data : np.ndarray, shape(n_samples,)
                    Single array of low frequency data.
    high_freq_data : np.ndarray, shape(n_samples)
                     Single array of high frequency data.
    
    Returns
    -------
    float
        Amount of phase amplitude coupling measured using phase lock value.
    """
...
def mi(low_freq_data, high_freq_data, phase_window_half_size = 10, phase_step_width = 20):
    """
    ...
    phase_window_half_size : int
                             Width of the phase window used for calculation of frequency/phase histogram.
                             Amplitude gets added to every phase bin within the window size.
                             Larger windows result in more smooth/increased PAC estimates.
    phase_step_width : int
                       Step width/shift of the phase window used for calculation of frequency/phase histogram.
...
def mvl(low_freq_data, high_freq_data):
    """
