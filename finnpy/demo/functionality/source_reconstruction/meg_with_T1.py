...
def calc_model(fs_path, anatomy_path, subj_name, t1_path, sensor_cov_path, cov_path, data_path,
...
def verify_integrity(morphed_epoch_data):
...
def main():
    ...
    model = calc_model(FS_PATH, ANATOMY_PATH, SUBJ_NAME, T1_SCAN_PATH + SUBJ_NAME + "/" + T1_SCAN_FILE, SENSOR_COV_PATH, ANATOMY_PATH + "sensor_cov/", DATA_PATH)
    sensor_data = dm.load(SENSOR_DATA_PATH)[:, 0, :] #Only the first epoch
    morphed_epoch_data = application(sensor_data, *model, FS_PATH)
    
    verify_integrity(morphed_epoch_data)
