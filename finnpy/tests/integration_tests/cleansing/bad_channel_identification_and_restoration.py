...
class test_bciar(unittest.TestCase):
    ...
    def test_bciar(self):
        ...
        (_, invalid_list, _) = bci.run(raw_data, ch_names, [frequency_sampling for _ in range(channel_count)], [[60, 100]], broadness = 3, visual_inspection = False)
        
        assert(len(invalid_list) == 1 and invalid_list[0] == 1)
        
        #Faulty channel gets substituted via neighbors
        rest_data = cr.run(raw_data, ch_names, invalid_list)
        
        ref_data = np.mean(np.concatenate((np.expand_dims(raw_data[0], axis = 1), np.expand_dims(raw_data[2], axis = 1), np.expand_dims(raw_data[6], axis = 1)), axis = 1), axis = 1)
        
        assert((np.abs(rest_data[1] - ref_data) < 0.001).all())
