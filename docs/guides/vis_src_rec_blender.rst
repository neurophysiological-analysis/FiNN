...
.. _vis_src_rec_blender_label:

Source reconstruction in Blender
================================
...
The code will have to be subtly adapted to use.
...
*REF_REGIONS* is a list of strings with names of all cortical regions relevant to the model. 

This codes iterates over all regions in the blender model. By default, these are named equally to the default region names from FiNNpy's source reconstruction, enabling re-identification.
...
The above functionality may be invoked via a single call to the following routine,
...
Changing perspective for rendering
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
       
While the model may be rendered from any angle, the provided blender-file comes with a set of pre-configured angles which can be chosen freely as demonstrated below. Pre-configured directions are as follows: "left_medial", "right_medial", "left_lateral", "right_lateral", "ventral", "dorsal", "frontal", and "occipital".
...
To output a 2D image, the 3D model must be rendered and the resulting image stored in a specific location. 
...
Likewise, this functionality may be triggered with a single call to
...
For convenience, the full blender code is available on GitHub or may be revealed by expanding the area below.
